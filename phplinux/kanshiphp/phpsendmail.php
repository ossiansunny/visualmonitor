...
function phpsendmail($hst, $prt, $from, $to, $subj, $body){
 $pgm="phpsendmail.php";
 $flg=getmailstatus();
 //writeloge('phpsendmail.php','flag:'.$flg);
 ///$flg=0; ///-----------------------------------debug  
 if ($flg==0){
  ...
  $mparam=$host.' '.strval($port).' '.$authuser.' '.$passwd.' '.$status; 
  writelogd('phpsendmail.php',$mparam);
  if ($status == '0' or is_null($status)) { /// 
    //writeloge('phpsendmail.php','if ok');
    /// post : integer
    /// body : array
    /// インスタンスを生成（true指定で例外を有効化）
    $mail = new PHPMailer(true);
    //writeloge('phpsendmail.php','new PHPMailer');
    $rcode=0;
    try {
      ///メールサーバー設定
      ///　デバック
      ...
      // smtpサーバー設定
      $mail->Host       = $host;
      //writeloge('phpsendmail.php','host set');
      /// SMTP認証     
      if ($port==587){             
        $mail->SMTPAuth   = true;
        /// -------------------------------------------------------
      ...
      $mail->isHTML(false);
      $mail->Body    = $bodystr;
      //writeloge('phpsendmail.php','body set');
      ...
      */    
      ///メール送信
      writeloge('phpsendmail.php','send');
      $mail->send();
      writeloge('phpsendmail.php','sended');
      $rcode=0;
...
// test
/*
$fm='vmadmin@sunnyblue.mydns.jp';
$too='oshima@sunnyblue.mydns.jp';
$rtncd=phpsendmail('192.168.1.139',587, $fm, $too, '添付テスト', '添付参照');
var_dump($rtncd);
*/
?>
