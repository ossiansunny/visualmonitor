...
///-------------------------------
  ...
  if ($action=='2' or $action=='3'){
    ...
    /// action=0(監視なし),1(Ping監視),5(Ncat監視)
  ...
  if ($updatesw!='0'){
    ...
    $mailSend="";
    $msg="";
    $event_sql="insert into eventlog values('".$host."','".$eventTime."','".$eventType."','".$snmpType."','".$snmpVal."','".$admin."','".$kanrimei."','".$kanrino."','".$cnfCls."','".$mailSend."','".$msg."')";
    putdata($event_sql); 
    ///-----------------------------------
    /// send mail
    ///-----------------------------------
...
///-------------------------------
  ...
  print "<option value='2'{$selOptArr[2]}>MacOSX</option>";
