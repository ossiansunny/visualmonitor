...
echo '<!--';
echo 'function check(host){';
echo 'if(window.confirm( host + " を削除してよろしいですか？")){';
echo 'return true;';
echo '}';
echo 'else{';
echo 'window.alert("キャンセルされました");';
echo 'return false;';
echo '}';
echo '}';
echo '// -->';
...
/// ホストデータ更新処理
  ...
  echo "<h2>▽　更新/削除 対象ホスト： {$host} 　▽</h2>";
  ///
  hostimagelist();
  ///
  echo '<h4>変更する箇所のみ入力して下さい</h4>';
  echo '<form name="updatedb" type="get" action="hostupdel.php">';
  echo '<table border=1>';
  echo '<tr><th>ホスト名</th><th>OS種類</th><th>結果</th><th>死活</th><th>表示名</th><th>メール</th><th>画像</th></tr>';
  echo '<tr>';

  echo "<td><input type=text name=host value={$host} readonly></td>";
  $ot = array('','','','');
  $ot[intval($ostype)]="selected";
  echo '<td><select name=ostype>';
  echo "<option value='0'{$ot[0]}>Windows</option>";
  echo "<option value='1'{$ot[1]}>Unix/Linux</option>";
  echo "<option value='2'{$ot[2]}>Gateway</option>";
  echo "<option value='3'{$ot[3]}>Others</option>";
  echo '</select></td>';
  echo "<td><input type=text name=result size=3 value={$result} readonly></td>";
  $ot=array('','','','','');
  $ot[intval($action)]="selected";
  echo '<td><select name=action>';
  echo "<option value='0'{$ot[0]}>監視なし</option>";
  echo "<option value='1'{$ot[1]}>PING監視</option>";
  echo "<option value='2'{$ot[2]}>SNMP監視</option>";
  echo "<option value='3'{$ot[3]}>SNMP通知なし</option>";
  echo "<option value='4'{$ot[4]}>Agent監視</option>";
  echo '</select></td>';
  $ot=array('','');
  echo "<td><input type=text name=viewname size=14 value={$viewname}></td>";
  $ot[intval($mailopt)]="selected";
  echo '<td><select name=mailopt>';
  echo "<option value='0'{$ot[0]}>非送信</option>";
  echo "<option value='1'{$ot[1]}>自動送信</option>";
  echo '</select></td>';
  //
  ...
  for ($cnt=0;$cnt<$rowcnt;$cnt++){
    ...
    if ($iitemlist[0]==$image){
      echo "<option value={$iitemlist[0]} selected>{$iitemlist[1]}</option>";
    }else{
  ...
  echo '<tr><th>TCPポート</th><th>CPU警告</th><th>メモリ警告</th><th>ディスク警告</th><th colspan="2">監視プロセス</th><th>SNMPコミュニティ</th></tr>';
  echo '<tr>';
  echo "<td><input type=text name=tcpport value={$tcpport}></td>";
  echo "<td><input type=text name=cpulim size=10 value={$cpulim}></td>";
  echo "<td><input type=text name=ramlim size=8  value={$ramlim}></td>";
  echo "<td><input type=text name=disklim size=10  value={$disklim}></td>";
  echo "<td colspan='2'><input type=text name=process  size=30 value={$process}></td>";
  echo "<td><input type=text name=comm size=10 value={$comm}></td>";
  echo "<input type=hidden name=fdata value={$data}>";    ##// all data
  echo "<input type=hidden name=oldaction value={$oldaction}>";    ##// old action
  echo '</tr>';
  echo '</table>';

  echo '<h4>ＴＣＰポート欄&emsp;&emsp;ポート番号；区切 <br>ＣＰＵ警告欄&emsp;&emsp;&emsp;警告値：危険値<br>メモリ警告欄&emsp;&emsp;&emsp;警告値：危険値<br>ディスク警告欄&emsp;&emsp;警告値：危険値<br>監視プロセス欄&emsp;&emsp;プロセス名；区切、インターネット内のサーバーは先頭に「&」</h4>';
  echo "<input type=hidden name=user value={$user}>";
  echo '&emsp;&emsp;&emsp;<input class=button type="submit" name="update" value="更新実行">';
  echo '</form>';
  echo '<br>';
  echo '<font color=red>*****「削除」を実行すると、ホスト情報が消えるので注意 *****</font><br>';
  ///
  echo '<form name="deletedb" type="get" action="hostupdel.php" onSubmit="return check(\''.$host.'\';">';
  echo "<input type=hidden name=user value={$user}>";
  echo "<input type=hidden name=host value={$host}>";
  echo "<td><input type=hidden name=fdata value={$data}></td>";  ##// all data
  echo '&emsp;&emsp;&emsp;<input class=buttondel type="submit" name="delete" value="削除実行">';
  echo '</form>';
  echo '<br>';
}
